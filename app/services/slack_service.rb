class SlackService
  ...
  def self.post_stached_image
    Typhoeus::Request.post("https://slack.com/api/chat.postMessage",
                                          # method: :post,
                                     params: {channel: mustache_request.channel,
                                              token: ENV['SLACK_BOT_TOKEN'],
                                              text: "With A Great Stache Comes Great Responsibility.",
                                              attachments: '[{"title":"' + mustache_request.user_full_name + ' just got stached!","image_url": "' + mustache_request.stached_user_image.url + '"}]'})
  end

  def self.post_headless_response(mustache_request)
    parse(Typhoeus::Request.new("https://slack.com/api/chat.postMessage",
                                          method: :post,
                                     params: {channel: mustache_request.channel,
                                              token: ENV['SLACK_BOT_TOKEN'],
